<template>
    <div class="wue-swipe" :style="{ height: height + 'px' }">
        <div class="wue-swipe-wrapper" ref="wrapper">
            <slot></slot>
        </div>
        <div class="wue-swipe-indicators" v-show="showIndicators">
            <div class="wue-swipe-indicator" v-for="(page, i) in pages" :key="page"
                 :class="{ 'is-active': i === index }"></div>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            reInitPages () {
                ...
                self.noDrag = children.length === 1 && self.noDragWhenSingle;
                
                let intDefaultIndex = Math.floor(self.defaultIndex);
                let defaultIndex = (intDefaultIndex >= 0 && intDefaultIndex < children.length) ? intDefaultIndex : 0;
                self.index = defaultIndex;

                children.forEach(function (child, index) {
                    self.pages.push(child.$el);
