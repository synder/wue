<template>
    <div class="wue-rater">
        <input v-model="currentValue" type="hidden">
        <a class="wue-rater-box" v-for="i in max"
           @click="handleClick(i-1)"
           :class="{'is-active':currentValue > i-1}"
           :style="style(i)">
            <span class="wue-rater-inner">{{star}}
                <span class="wue-rater-outer"
                      :style="{color: color, width: cutPercent + '%'}"
                      v-if="cutPercent > 0 && cutIndex === i-1">{{star}}</span>
            </span>
        </a>
    </div>
</template>

<script>
    export default {
        name: 'wue-rater',
        created () {
            this.currentValue = this.value;
        },
        mounted () {
            this.updateStyle()
        },
        props: {
            max: {
                type: Number,
                default: 5
            },
            value: {
                type: Number,
                default: 0
            },
            disabled: Boolean,
            star: {
                type: String,
                default: '★'
            },
            color: {
                type: String,
                default: '#FFBE00'
            },
            margin: {
                type: Number,
                default: 2
            },
            size: {
                type: Number,
                default: 25
            }
        },
        computed: {
            sliceValue () {
                const _val = this.currentValue.toFixed(2).split('.');
                return _val.length === 1 ? [_val[0], 0] : _val
            },
            cutIndex () {
                return this.sliceValue[0] * 1
            },
            cutPercent () {
                return this.sliceValue[1] * 1
            },
        },
        methods: {
            handleClick (i, force) {
                if (!this.disabled || force) {
                    if (this.currentValue === i + 1) {
                        this.currentValue = i;
                        this.updateStyle()
                    } else {
                        this.currentValue = i + 1
                    }
                }
            },
            updateStyle () {
                for (var j = 0; j < this.max; j++) {
                    if (j <= this.currentValue - 1) {
                        this.$set(this.colors, j, this.color)
                    } else {
                        this.$set(this.colors, j, '#ccc')
                    }
                }
            },

            style(i){
                return {
                    color: this.colors && this.colors[i - 1] ? this.colors[i - 1] : '#ccc',
                    marginRight: this.margin + 'px',
                    fontSize: this.size + 'px',
                    width: this.size + 'px',
                    height: this.size + 'px',
                    lineHeight: this.size + 'px'
                };
            }
        },
        data () {
            return {
                colors: [],
                currentValue: 0
            }
        },
        watch: {
            currentValue (val) {
                this.updateStyle();
                this.$emit('input', val)
            },
            value (val) {
                this.currentValue = val
            }
        }
    }
</script>

<style scoped lang="less">

    @import "../../styles/base/reset.less";
    
    .wue-rater {
        text-align: left;
        display: inline-block;
        line-height: normal;
    }

    .wue-rater a {
        display: inline-block;
        text-align: center;
        cursor: pointer;
        color: #ccc;
    }

    .wue-rater a:last-child {
        padding-right: 2px !important;
        margin-right: 0px !important;
    }

    .wue-rater a:hover {
        color: #ffdd99;
    }

    .wue-rater a.is-disabled {
        color: #ccc !important;
        cursor: not-allowed;
    }

    .wue-rater-box {
        position: relative;
    }

    .wue-rater-inner {
        position: relative;
        display: inline-block;
    }

    .wue-rater-outer {
        position: absolute;
        left: 0;
        top: 0;
        display: inline-block;
        overflow: hidden;
    }
</style>
